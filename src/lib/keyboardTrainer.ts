...
export function normalizeTrainerSettings(value: unknown): KeyboardTrainerSettings {
  ...
  const focusWeak =
    typeof v.focusWeak === 'boolean' ? v.focusWeak : DEFAULT_KEYBOARD_TRAINER_SETTINGS.focusWeak
  return { mode, fromOctave, toOctave, accidentals, focusWeak }
}
...
/**
 * Per-pitch-class spaced-repetition schedule for this tool (`mt:srs:keyboard-trainer`).
 * Keyed by `srsKeyForPc`. Tests build their own via `createSrsStore`.
 */
export const keyboardSrsStore = createSrsStore('keyboard-trainer')
