...
- [x] Piano exercises: 5-finger patterns, scale fingerings with correct finger numbers displayed on Keyboard
...
- [x] Sound audit: replace the plain dual-oscillator synth with more natural instrument voices — evaluate Karplus-Strong string synthesis for bass/guitar pluck, FM or additive synthesis for a piano-like tone, per-tool voice selection (which tool uses which voice), and consistent levels across voices; self-hosted tiny samples only as a last resort (must stay offline/CSP-clean, no CDN or runtime fetching)
- [x] Per-tool voice context wiring: the engine now supports `fretted`/`keyboard` voice preferences and `setVoiceContext('fretted'|'keyboard')`, but pages don't yet call it — so all tools currently use the `fretted` voice (default pluck) except the Settings test buttons. Wire `engine.setVoiceContext('keyboard')` (or pass `voice:`) in the keyboard tools (KeyboardNoteTrainer, piano exercises, EarTraining piano contexts) so keyboard tools use the `keyboard` voice preference; optionally let the fretboard tools honor the instrument (bass vs guitar) when picking pluck brightness
- [x] Mobile/touch audit: 44px minimum tap targets on fretboard/keyboard hit areas, remove hover-only affordances, touch-action on controls, verify each tool at 390px width
...
- [x] Apply sharps/flats preference in remaining tools (currently only default Fretboard/Keyboard note-name labels honor it; tools that pass explicit spelled labels — scales, chords, note reading, etc. — still ignore the global preference)
- [x] Front-page rework: the home grid has grown to 14 uniform tiles — group tools into sections (e.g. Learn / Train / Play / Reference), surface the practice dashboard's streak + suggested routine at the top, show recently-used tools first within their group, give Settings a distinct low-key placement (e.g. header gear icon), keep it scannable on a phone (390px)
- [x] Custom tuning editor: define and save arbitrary tunings (name + per-string pitch) alongside the built-ins

## M7 — Sound & play-along feedback (user feedback 2026-07-23)

- [x] Instrument voice quality rework — direct user feedback: "I really really don't like the sounds for bass and guitar, they are way too clangy and don't sound natural at all"; the piano is "okay but could be better". (1) Rework the Karplus-Strong pluck to sound like a plucked string, not metal: low-pass-filtered (darker) excitation burst instead of raw white noise, pick-position comb filter, gentler loop damping tuned per register (bass warm and round, sustained; guitar brighter but never clangy), remove inharmonic ring/zing, soft attack rounding, optional light body-resonance stage; verify with quantitative spectral tests (spectral centroid targets per register, no energy spikes above ~5kHz for bass, smooth exponential decay) since we can't listen in CI. (2) Refine the piano voice: better partial balance, hammer-like attack transient, register-dependent brightness, longer natural decay. (3) Circle of Fifths should use the piano voice (setVoiceContext('keyboard')). Keep 'classic' available; keep levels matched.
- [x] Play-along expansion — user feedback: drums are too quiet; wants more grooves, more accompaniment styles, and an accompaniment volume slider. (1) Raise/balance drum levels against the comp and metronome (drums should feel like the backbone, not background). (2) Add 5-6 new grooves (e.g. disco four-on-the-floor, motown/soul, reggae one-drop, train beat, half-time shuffle, 16th-funk variant) in the existing 16-step format. (3) Add 2-3 new accompaniment styles beyond pad/stabs (e.g. arpeggiated 8ths, offbeat comp/skank, block-chord rhythm). (4) Mix controls in the Play-Along UI: volume slider for the accompaniment AND one for the drums, persisted in playAlongSettings (version bump + migration).
- [x] Play-along key mode (bug/gap from user feedback): the key picker selects only a root and `resolveDegrees` hardcodes 'major' (src/lib/accompaniment.ts) — progressions are silently wrong for minor practice. Add a Major / Natural minor / Harmonic minor mode switch next to the key picker: degrees resolve via buildDiatonicChordCards(rootPc, mode) (already supports all three); numerals/symbols must reflect the mode's actual qualities (i, ii°, III… in minor); add minor-appropriate progression presets (i-VI-III-VII, i-iv-v, i-iv-V (harmonic), 12-bar minor blues) and show presets fitting the selected mode; key spelling follows the minor key's signature (relative major, e.g. A minor → no accidentals, G minor → 2 flats); chord-tones fretboard display and the big chord display follow automatically via the resolved chords; persist mode in playAlongSettings (version bump + migration + tests). Pentatonics intentionally excluded: a 5-note scale defines no diatonic chord set — pentatonic practice happens over major/minor progressions (documented here as the decision).
